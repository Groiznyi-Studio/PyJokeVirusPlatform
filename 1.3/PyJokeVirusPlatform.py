from ctypes import windll, byref, c_bool, wintypes
...
def Beep(height,time):
    ...
    Example:
    1)Beep(500,1000) - make a sound with a height of 500 for 1 second
    2)Beep(2000,500) - make a sound with a height of 2000 for 0.5 second"""
...
def Sleep(time):
    ...
    Example:
    1)Sleep(1000) - stop the program for 1 second
    2)Sleep(6000) - stop the program for 6 second
    3)Sleep(500) - stop the program for 0.5 second
    """
    kernel = windll.LoadLibrary("kernel32.dll")
...
def RaiseCriticalError():
    ...
    windll.ntdll.NtRaiseHardError(0xc0000022, 0, 0, 0, 6, byref(wintypes.DWORD()))
...
def MessageBox(text,title,icon):
    ...
    Example:
    1).MessageBox(\"Function: MessageBox\",\"WARNING\",WARNING_ICON) - displays a message \"Function: MessageBox\", with a title \"WARNING\", icon warning

    2)MessageBox(\"Function: MessageBox\",\"ERROR\",ERROR_ICON) - displays a message \"Function: MessageBox\", with a title \"ERROR\", icon error

    3)MessageBox(\"Function: MessageBox\",\"INFO\",INFO_ICON) - displays a message \"Function: MessageBox\", with a title \"INFO\", icon info
    
    4)MessageBox(\"Function: MessageBox\",\"QUESTION\",QUESTION_ICON) - displays a message \"Function: MessageBox\", with a title \"QUESTION\", icon question"""
    if icon == "ERROR_ICON":
...
def MessageBeep(number):
    ...
    Example:
    1)MessageBeep(MB_ERROR)
    2)MessageBeep(MB_DEFAULT)"""
...
class Block_Processes(Thread):
    ...
    Example:
    1)Block_Processes([\"notepad.exe\", \"explorer.exe\"], 0.5, 50).init() - block notepad.exe and chrome.exe processes, stop scanning for 0.5 seconds, and do it 50 steps, .init() - start"""
...
def OpenUrl(url):
    ...
    Example:
    1)OpenUrl(\"http://google.com\") - open url google.com, http protocol
    2)OpenUrl(\"https://google.com\") - open url google.com, https protocol"""
    if url[0:7] == "http://":
...
def SetStatusCD(status):
    ...
    Example:
    1)SetStatusCD(True) - open CD drive
    2)SetStatusCD(False) - close CD drive"""
    if status == True:
...
def GetDate(section):
    """Function that receives the date. Accepts arguments like this:
    1)str or list section: indicates what to get.("year", "month", "day", "hour", "minute", "second")
    
    What returns:
    1)If section is str, it returns what you specified.
    2)If section is a list, it returns what you specified only as a list
    3)If you specified an invalid section argument, it returns a SectionError
    
    Example:
    1)GetDate("year") - returns 2020
    2)GetDate("day") - returns 31
    3)GetDate(["year","month","day"]) - returns [2020, 12, 31]"""
    ...
    if type(section) == list:
        ...
        for i in section:
            if i.lower() == "day":
                result.append(all_date.day)
            elif i.lower() == "month":
                result.append(all_date.month)
            elif i.lower() == "year":
                result.append(all_date.year)
            elif i.lower() == "hour":
                result.append(all_date.hour)
            elif i.lower() == "minute":
                result.append(all_date.minute)
            elif i.lower() == "second":
                result.append(all_date.second)
            else:
                status = 1
                break
        if status == 0:
    ...
    elif section.lower() == "day":
        return all_date.day
    elif section.lower() == "month":
        return all_date.month
    elif section.lower() == "year":
        return all_date.year
    elif section.lower() == "hour":
        return all_date.hour
    elif section.lower() == "minute":
        return all_date.minute
    elif section.lower() == "second":
        return all_date.second
    else:
        return "SectionError"
